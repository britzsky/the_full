// =====================================================================
// 영수증 마감 자료(개인구매) 탭 - 데이터 훅
// - AccountPersonPurchaseTallyListAll (account_id 무관 전체 조회, type=1008)
...
// 영수증 타입 코드 → 한글 라벨 매핑
// (accountpersonpurchasesheet.js의 RECEIPT_TYPES와 동일한 코드 체계)
...
// receipt_type 값 정규화 (accountpersonpurchasesheet.js와 동일한 로직)
...
// =====================================================================
// 개인구매(type=1008) 영수증 마감 자료 조회 훅
// - /Account/AccountPersonPurchaseTallyListAll 1회 호출
// - filters: { year, month, receiptType, accountId }
// =====================================================================
export default function usePersonPurchaseReceiptArchiveData() {
  ...
  const fetchRows = useCallback(async (filters) => {
    setLoading(true);
    try {
      const res = await api.get("/Account/AccountPersonPurchaseTallyListAll", {
        params: {
      ...
      console.error("개인구매 영수증 마감 자료 조회 실패:", err);
      setRows([]);
      return [];
    } finally {
      setLoading(false);

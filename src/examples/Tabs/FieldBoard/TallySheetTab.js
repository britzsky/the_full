...
function TallySheet() {
  ...
  }, [localAccountId, selectedAccountId, accountList, setSelectedAccountWithLoading]);
  ...
  useEffect(() => {
    ...
    const list = filteredAccountList || [];
    if (list.length === 0) {
      if (selectedAccountId) setSelectedAccountWithLoading("");
      return;
    }

    const hasSelected = list.some((row) => String(row.account_id) === String(selectedAccountId));
    if (!hasSelected) {
      setSelectedAccountWithLoading(String(list[0].account_id));
    }
  }, [filteredAccountList, selectedAccountId, localAccountId, setSelectedAccountWithLoading]);
        ...
        <Card>
          <MDBox p={2}>
            ...
            <TextField
              ...
              helperText={loadingTallyNote ? "메모를 불러오는 중입니다." : "읽기 전용"}
            />
          </MDBox>
        </Card>
      </MDBox>

...
export default function useAccountCorporateCardData() {
  ...
  const fetchAccountCorporateCardPaymentList = useCallback(
    async ({ year, month, account_id }) => {
      return withLoading(async () => {
        try {
          const res = await api.get("/Account/AccountPersonPurchaseTallyList", {
  ...
  const fetchAccountCorporateCardPaymentDetailList = useCallback(
    async ({ sale_id, account_id, saleDate }) => {
      return withLoading(async () => {
        try {
          const res = await api.get("/Account/AccountPersonPurchaseDetailList", {
            params: { sale_id, account_id, saleDate },
          });

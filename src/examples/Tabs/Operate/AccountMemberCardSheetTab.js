...
function AccountMemberSheet() {
  ...
  // =========================
  // ✅ 통합/유틸이면 account_id 강제 (저장/행추가/직책변경 공통 보정)
  // =========================
  const applyUtilAccountId = useCallback(
    (rows) => {
      ...
      return list.map((r) => {
        ...
        // 통합/유틸이 아닌데 account_id가 1/2로 남아있으면 selectedAccountId로 복구
  ...
  // 행 삭제 처리: 신규 행(idx=null)은 로컬 제거, 기존 행은 백엔드 del_yn=Y 처리
  const handleWsDeleteRow = async () => {
    const { rowIndex, idx } = wsCtxMenu;
    closeWsCtxMenu();

    if (idx == null) {
      // 아직 저장 안 된 신규 행은 로컬에서만 제거
    ...
    try {
      ...
      // 삭제 후 목록 새로고침 (del_yn=Y 행은 조회 제외)
      const latest = await fetchWorkSystemList({ snapshot: true });
      setWsRows(latest || []);
      setWsOriginal(latest || []);
    } catch (err) {

...
// - 조회: /Account/HeadOfficeCorporateCardPaymentList
// - 저장: /Corporate/receipt-scan
...
function TallySheet() {
  ...
  useEffect(() => {
    ...
    if (!selectedAccountId) {
      if (accountListCheckedRef.current && (filteredAccountList || []).length === 0) {
  ...
  const handleOtherReceiptFileChange = (e) => {
    ...
    setOtherForm((p) => ({
      ...p,
      receipt_image: files[0] || null,
      receipt_files: files,
      receipt_image2: files.length >= 2 ? "" : p.receipt_image2,
      receipt_image3: files.length >= 3 ? "" : p.receipt_image3,
    }));
    setOtherReceiptPreview(previewList[0] || null);
    setOtherReceiptPreviewList(previewList);
  };

  useEffect(() => {
    return () => {
      (otherReceiptPreviewList || []).forEach((previewUrl) => {
  ...
  const selectOtherRowForInlineEdit = (rowObj, rowKey) => {
    ...
    setOtherForm({
      ...
      total: String(rowObj.total ?? ""),
      receipt_image: null,
      receipt_image2: String(rowObj.receipt_image2 || ""),
      receipt_image3: String(rowObj.receipt_image3 || ""),
      receipt_files: [],
      receipt_type: rowObj.receipt_type || "UNKNOWN",
      sale_id: String(rowObj.sale_id ?? ""),
      account_id: String(rowObj.account_id ?? selectedAccountId ?? ""),
  ...
  const initialForm = {
    ...
    add_yn: "N",
    add_name: "",
    // ✅ 필수값 검사(!formData[key])에 걸리지 않도록 문자열로 보관 ("0"|"1"|"2")
    account_type: "0",
  };
  ...
  const handleSubmit2 = async () => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        // 거래처 유형은 저장 시점에 숫자로 변환
        account_type: Number(formData.account_type),
        bank_image: bankPath,
        biz_image: bizPath,
        del_yn: "N",
        user_id: localUserId,
  ...
  const saveUseInputYn = useCallback(
    async (inputYn) => {
      ...
      const rawMatchedIdx =
        (useList || []).find((u) => {
          const rowYear = Number(u?.count_year ?? u?.year);
          const rowMonth = Number(u?.count_month ?? u?.month);
          const rowType = String(u?.type ?? u?.use_type ?? u?.gubun ?? "").trim();
          const rowAccountId = String(u?.account_id ?? u?.row_account_id ?? "").trim();
      ...
      const matchedIdxNum = Number(rawMatchedIdx);
      const safeIdx =
        Number.isInteger(matchedIdxNum) && matchedIdxNum > 0 ? matchedIdxNum : null;
      ...
      try {
        ...
        const matched = refreshed.find((r) => {
    ...
    [canManageUseInputState, useMenu.target, selectedAccountId, closeUseMenu, fetchUseList, useList]
  );
          ...
          <Grid container spacing={1} alignItems="center" sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={4} sm={3}>
              <Select
                fullWidth
                size="small"
                value={formData.account_type ?? "0"} // 거래처 유형 기본 문자열 "0"
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    // 필수값 로직 유지 위해 문자열로 저장
                        ...
                        >
                          ...
                          {previewSrc && (
                            <MDButton
                              ...
                              onClick={(ev) => {
                                ev.stopPropagation();
                                const selectedFiles = Array.isArray(fileInfo?.files)
                                  ? fileInfo.files.filter(Boolean).slice(0, 3)
                                  : [];
                                openFloatingPreview(
                                  selectedFiles.length ? selectedFiles : fileInfo?.file || previewSrc,
                                  "(법인카드) 영수증 미리보기"
                                );
                              }}
                            >
              ...
              onClick={async () => {
                ...
                const changed = (cardEditRows || [])
                  .map((r, idx) => {
                    ...
                    const hasFile = Array.isArray(cardRowFiles?.[rowKey]?.files)
                      ? cardRowFiles[rowKey].files.length > 0
                ...
                try {
                  ...
                  for (let i = 0; i < changed.length; i++) {
                    ...
                    const selectedFiles = Array.isArray(cardRowFiles?.[rowKey]?.files)
                      ? cardRowFiles[rowKey].files.filter((f) => f instanceof File).slice(0, 3)
                      : [];
                    const uploadFiles =
              ...
              <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                ...
                {otherReceiptPreview && (
                  <MDButton
                    ...
                    size="small"
                    onClick={() => {
                      const selectedFiles = Array.isArray(otherForm.receipt_files)
                        ? otherForm.receipt_files.filter(Boolean).slice(0, 3)
                        : [];
                      const previewSources = selectedFiles.length
                        ? selectedFiles
                        : [otherForm.receipt_image || otherReceiptPreview].filter(Boolean);
                      openFloatingPreview(previewSources, "(기타) 영수증 미리보기");
                    }}
                  >
                    미리보기
                  </MDButton>

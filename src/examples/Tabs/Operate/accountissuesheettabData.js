...
export default function useAccountIssueSheetData(teamCode = 1) {
  ...
  // 입력 행의 거래처 정보를 "집계 키/표시명" 형태로 표준화
  ...
  // 거래처 검색 입력 후 Enter 시 첫 일치값 자동 선택
  ...
  // 거래처 셀 클릭 시 선택 상태만 변경하고, 드롭다운은 사용자가 직접 열도록 유지
  ...
  const handleSave = async () => {
    ...
    if (invalidRows.length > 0) {
      Swal.fire("필수값 확인", "접수일/거래처/구분은 필수입니다.", "warning");
      return;
    }
